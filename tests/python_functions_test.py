...
class TestPythonFunctionPractice(unittest.TestCase):
  ...
  def test_volume_of_cube(self):
    vol_of_cube_with_side_length_four = volume_of_cube(4)
    self.assertEqual( 64, vol_of_cube_with_side_length_four )

  #Given a String, return the String reversed
  def test_reverse_string(self):
     string_reversed = reverse_string("cakes")
     self.assertEqual("sekac", string_reversed)
    

  #Given a value in farenheit, convert this into celsius.
  def test_fahrenheit_to_celsius(self):
      five_farenheit_in_celcius = farenheit_to_celcius(5)
      self.assertEqual(-15, five_farenheit_in_celcius)
